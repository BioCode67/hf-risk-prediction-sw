...
children.push(
  ...
  ...FIG("vitals_baseline_concept.png", 0.364, "그림 1. 개인 기저선 이탈의 개념 — 같은 절대값이 환자에 따라 다른 의미를 갖는다", 555),
  P([
    { t: "위 그림이 본 제안의 출발점이다. 두 환자 모두 호흡수가 22회/분에 도달했고, 병동 공통 임계값(20회/분)은 " },
    { t: "두 환자 모두에게 경보한다", b: true },
    { t: ". 그러나 환자 A는 평소 14회/분에서 8회 상승한 실제 악화이고, 환자 B는 평소 21회/분인 환자가 1회 변동한 것에 불과하다. " },
    { t: "환자 B에게 울린 경보가 바로 본 과제가 제거하려는 오경보다.", b: true },
    { t: " 개인 기저선 이탈 피처는 이 구분을 모델 입력 단계에서 만들어 준다." },
  ]),
  H2("4.2 설명가능성(XAI) 내장"),
  ...
  P([
    ...
    { t: "로 전환한다 — 안정기를 음성, 심정지 직전 구간을 양성으로 하여 환자 내부에서 라벨을 부여한다. 동시에 이 구조가 만들어내는 측정의 한계를 명시하고 보정 경로를 설계에 포함한다(제5-2장)." },
  ]),
  ...FIG("vitals_labeling_concept.png", 0.400, "그림 2. within-patient 라벨링 — 대조군 없이 같은 환자의 시간축에서 양성·음성을 만든다", 555),
  P([
    { t: "각 윈도우는 과거 8시간을 관찰하고, 그 종료 시점 이후 1시간 안에 심정지가 발생하면 양성이 된다. 심정지 수 시간 전의 안정 구간은 " },
    { t: "같은 환자로부터 얻는 음성 표본", b: true },
    { t: "이며, 개인 기저선은 입원 초기 6시간에서 산출한다. 즉 대조군 환자를 확보하지 못하는 데이터 구조가 오히려 개인화 설계와 자연스럽게 맞물린다 — 비교 대상이 다른 환자가 아니라 그 환자의 과거이기 때문이다." },
  ]),
...
children.push(
  ...
  NUM(1, [
    ...
    { t: "심정지 미발생 환자를 포함한 공개 데이터(MIMIC-IV)로 안심존 외부에서 모델을 사전학습하고, 그 모델을 안심존에 들여와 경북대 데이터로 추가 학습·보정한다. 대조군에서 학습한 오경보 특성을 case-only 데이터에 이식하는 설계다. 대구 안심존은 본선 전에 사용할 패키지 목록과 모델을 사전 통보하는 절차를 따르므로(제9-1장), 사전학습 모델을 그 목록에 포함해 신고한다. 절차상 성립하지 않을 경우 경로 2 또는 안심존 내부 학습 단독으로 전환하며, 어느 쪽이든 본 설계는 유지된다." },
  ]),
  NUM(2, [
...
children.push(
  ...
  P([
    { t: "활력징후 시계열을 정제해 8시간 슬라이딩 윈도우로 자르고, 윈도우마다 통계 피처와 개인 기저선 이탈 피처를 생성한 뒤, 예측 지평 안에 심정지가 들어오는지로 within-patient 라벨을 부여해 cost-sensitive XGBoost를 학습한다. 평가는 임상 표준 NEWS를 비교군으로 두고 수행하며, 모든 경보에 SHAP 기여 요인을 함께 산출한다. " },
    { t: "★ 표시한 두 단계가 본 제안의 차별 지점", b: true },
    { t: "이며, 각각 제4-1장과 제4-3장에서 다룬 내용이다. 단계별 상세 명세는 제6-4장에 있다." },
  ]),
  H2("6.2 피처 설계"),
  ...
  P("이 피처가 핵심 차별점인 만큼, 주장에 그치지 않도록 통제 실험으로 측정하였다. 동일한 환자 분할·동일한 하이퍼파라미터로 두 번 학습하고 피처 집합만 바꾼다. 단일 분할은 결과가 아니므로 서로 다른 환자 분할 시드 5회로 반복하였다."),
  TABLE(
  ...
  P("평균 AUPRC는 0.784 → 0.834로 상대 6.6% 상승했고, 5회 중 4회에서 개선되었다. 그러나 더 주목할 것은 평균이 아니라 분산이다.", { before: 140 }),
  CALLOUT([
    { t: "개인화 피처를 제외하면 시드 간 표준편차가 4.7배로 커진다(0.008 → 0.037). ", b: true },
    { t: "즉 이 피처의 기여는 “성능을 얼마나 올리는가”보다 “어떤 환자가 평가 집합에 들어오든 성능이 유지되는가”에 있다. 개인화 없이는 모델이 활력징후의 절대값을 학습하므로 평가 환자군의 기저 분포에 성능이 좌우되고, 개인화가 들어가면 환자 간 비교가 가능한 " },
    { t: "이탈량", b: true },
    { t: "을 학습하므로 그 의존이 줄어든다. 이는 제4-1장에서 주장한 병원 간 전이성의 직접적 근거다." },
  ]),
  P([
    { t: "반대 방향의 관찰도 함께 밝힌다. 5회 중 1회(시드 42)에서는 개인화 피처가 오히려 AUPRC를 1.4% 낮췄다. " },
    { t: "단일 시드만 확인했다면 정반대 결론에 도달했을 것이며", b: true },
    { t: ", 이것이 본 과제가 단일 실행 결과를 성능으로 주장하지 않는 이유다. 또한 공개 실데이터(패혈증)에서는 이 피처가 상위 기여 요인이 아니었다(제7-3장) — 사건 유형과 데이터에 따라 유효성이 달라진다는 뜻이며, 심정지 실데이터에서의 재검증을 제9-5장에 수행 항목으로 명시하였다." },
  ]),
  P("본 실험은 `scripts/ablate_personalized.py`로 재현 가능하다.", { size: 18, color: MUTED }),
  H2("6.3 학습 설계와 누수 방지"),
...
children.push(
  ...
  P("위 수치는 본 제안서의 그림 4~7을 생성한 실행 기준이다. 합성 코호트를 재생성하면 AUPRC는 XGBoost 0.77~0.97, NEWS 0.42~0.63 범위에서 변동한다.", { size: 18, color: MUTED, before: 100, after: 120 }),
  P([
  ...
  ...FIG("vitals_pr_curve.png", 0.855, "그림 4. PR 곡선 비교 — ROC로는 보이지 않는 격차가 정밀도–재현율 평면에서 드러난다", 360),
  ...FIG("vitals_alarm_burden.png", 0.712, "그림 5. 동일 검출률에서의 알람 부담 — 고민감도 구간에서 NEWS의 알람이 급증한다", 420),
  ...FIG("vitals_trajectory.png", 0.544, "그림 6. 환자 악화 궤적과 경보 시점 — 위험도 상승이 사건에 선행한다", 510),
...
children.push(
  ...
  NUM(2, [
    ...
    { t: "실데이터 전역 기여도에서 4위·6위이며, 1~3위는 호흡수·체온의 절대값이다. 합성 심정지 데이터의 통제 실험(제6-3장)에서는 평균 6.6% 기여했으나, 실데이터에서 상위 요인으로 재현되지는 않았다. “기여한다”까지가 정확한 표현이며 “핵심 요인”은 과장이다." },
  ]),
  NUM(3, [
...
children.push(
  ...
  P([
    { t: "“알람이 40% 줄었다”는 표현은 그 자체로는 체감되지 않는다. 병동 규모로 환산하면 달라진다. 제7-1장에서 실측한 검출률 50% 지점의 알람 부담(윈도우 100개당 본 모델 24.8건, NEWS 41.3건)을 " },
    { t: "시간당 1회 평가하는 병동", b: true },
    { t: "에 적용하면, 30병상 병동은 하루 720회를 평가하므로 NEWS 기준 297건, 본 모델 기준 179건의 알람이 발생한다." },
  ]),
  ...FIG("vitals_ward_burden.png", 0.417, "그림 10. 병동 규모별 일일 알람 건수 — 검출률을 고정한 상태에서의 차이", 540),
  P([
    { t: "30병상 병동 기준 하루 약 " },
    { t: "118건", b: true },
    { t: "의 알람이 사라진다. 간호사 한 교대(8시간)로 환산하면 약 40건이며, 이는 " },
    { t: "같은 수의 위험 환자를 잡아내면서", b: true },
    { t: " 얻는 감소다. 검출률을 낮춰 알람을 줄인 것이 아니라는 점이 핵심이다." },
  ]),
  CALLOUT([
    { t: "이 환산의 전제. ", b: true, c: WARN },
    { t: "위 수치는 패혈증 공개 데이터(양성률 1.19%)에서 측정한 알람 부담을 산술적으로 확대한 것이며, 실제 병동의 환자 구성·평가 주기·심정지 발생률에 따라 달라진다. 심정지에서의 실측치는 본선에서 산출한다. " },
    { t: "여기서 유효한 것은 절대 건수가 아니라, 검출률을 유지한 채 알람을 줄이면 그 감소분이 병동 규모에 비례해 누적된다는 구조다.", b: true },
  ], "FBF2F2", WARN),
  H2("8.2 임상 운영 흐름에의 통합"),
  P("본 시스템은 기존 신속대응체계(RRT)를 대체하지 않고 그 앞단에 놓인다. 활력징후가 기록되는 시점마다 위험도를 산출하고, 임계값을 넘으면 기여 요인과 함께 알린다. 이때 근거가 함께 제시되므로 대응이 달라진다."),
  BULLET([{ t: "무엇을 확인할지가 정해진다. ", b: true }, { t: "“위험도 0.7”만 제시되면 무엇부터 볼지 알 수 없다. “호흡수 상승 추세와 SpO₂ 하강이 주 기여”가 함께 오면 확인 순서가 정해진다." }]),
  BULLET([{ t: "표현형에 따라 대응이 갈린다. ", b: true }, { t: "제4-4장의 호흡성·순환성 아형은 지배적 악화 계통이 다르므로, 초기 대응(산소화 평가 대 순환 평가)의 우선순위도 달라진다." }]),
  BULLET([{ t: "경보를 신뢰할지 판단할 수 있다. ", b: true }, { t: "근거가 임상 판단과 어긋나면 의료진이 그 경보를 기각할 수 있다. 기각 가능한 경보만이 신뢰받는 경보다." }]),
  H2("8.3 방법론의 재사용성"),
  P([
    { t: "본 제안의 핵심 설계인 " },
    { t: "case-only 코호트의 within-patient 전환", b: true },
    { t: "은 경북대 데이터에만 해당하는 기법이 아니다. 사건이 발생한 환자만 수집된 데이터는 의료 분야에서 드물지 않으며, 특히 미개방·희귀사건 데이터가 그러하다. 대조군 확보가 어렵다는 이유로 활용되지 못한 데이터에 동일한 설계를 적용할 수 있다." },
  ]),
  P("또한 “동일 검출률에서의 알람 부담”이라는 평가 축은 심정지에 국한되지 않는다. 패혈증·급성신손상·섬망 등 희귀사건 조기경보 전반에서 AUROC가 오경보를 은폐하는 동일한 문제가 발생하므로, 본 과제가 제시하는 평가 프레임은 그대로 이전된다."),
  H2("8.4 기대효과 정리"),
  TABLE(
    [2100, 6926],
    ["영역", "기대효과"],
...
children.push(
  H1("9. 추진 계획 및 안심존 활용"),
  P("안심존은 오프라인 폐쇄망이며, 사용할 분석환경은 사전에 확정해야 하고 결과물 반출에는 심의를 거쳐야 한다. 대회 공고와 운영기관 확인을 통해 파악한 아래 제약을 전제로 계획을 수립하였다."),
  H2("9.1 확인된 운영 조건"),
  P([
    { t: "본 과제가 선택한 데이터는 [대구] 경북대학교병원 활력징후 데이터이므로, 본선 분석은 " },
    { t: "계명대학교 동산의료원 의료데이터 안심활용센터", b: true },
    { t: "에서 수행한다(평일 08:30~17:30). 예선 기간의 샘플 확인은 건양대학교의료원 의료데이터안심존을 방문해 완료하였다." },
  ]),
  TABLE(
    [3600, 5426],
    ["항목", "조건"],
  ...
  CALLOUT([
    { t: "대구 안심존은 “반입 신청”이 아니라 “사전 통보” 방식이다. ", b: true },
    { t: "패키지를 신청해 들여오는 대전과 달리, 대구·광주는 본선 전에 사용할 패키지 목록과 모델을 미리 알려 환경에 반영하는 절차를 따른다. 따라서 " },
    { t: "본선 진출이 확정되는 즉시 목록을 확정해 통보하는 것이 임계경로", b: true },
    { t: "이며, 뒤늦게 추가하려 해도 설치가 불가능하다. 본 과제가 의존성을 6개로 최소화한 것은 이 절차를 통과시키기 위한 설계이기도 하다." },
  ], "FBF2F2", WARN),
  ...
  P("본선에서 무엇을 확인할 것인지, 그리고 무엇을 확인하면 성공으로 볼 것인지를 사전에 정의한다. 순서는 의존 관계를 따르며, 앞 단계가 성립해야 다음 단계가 의미를 갖는다."),
  TABLE(
    [700, 2900, 5426],
    ["단계", "확인할 것", "성공 기준"],
    [
  ...
  CALLOUT([
    { t: "기대와 다른 결과도 결과로 보고한다. ", b: true },
    { t: "특히 3단계는 본 제안의 핵심 차별점에 대한 검증이며, 공개 데이터에서 이미 기대에 못 미치는 결과가 나온 항목이다. 심정지에서도 재현되지 않는다면 그 사실 자체가 " },
    { t: "“개인 기저선 이탈이 어떤 사건에서 유효하고 어떤 사건에서는 그렇지 않은가”", b: true },
    { t: "라는 답이 되며, case-only 미개방 데이터를 다루는 후속 연구에 그대로 쓰인다. 유리한 결과만 보고하는 것은 본 과제의 방법론적 태도와 맞지 않는다." },
  ]),
  H2("9.6 폐쇄망 대응"),
  P("사전 통보할 패키지는 numpy, pandas, scikit-learn, xgboost, shap, matplotlib 6종으로 최소화한다. 대구 안심존은 사전 구축 환경만 제공하고 추가 설치가 불가능하므로, 목록에 없는 의존성이 뒤늦게 필요해지는 상황을 만들지 않는 것이 설계 원칙이다. 외부 다운로드에 의존하는 구성요소는 두지 않으며, 외부 API가 필요한 기능(자연어 생성 등)은 안심존 대상에서 제외하고 결정론적으로 계산되는 근거 제시만을 사용한다."),
