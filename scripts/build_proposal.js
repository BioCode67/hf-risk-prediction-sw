...
const FIG = (file, ratio, caption, widthPx = 560) => [
  new Paragraph({
...
const PAGES = {
  summary: 3, s1: 4, s2: 5, s3: 6, s4: 7, s5: 8, s52: 8, s6: 10,
  s7: 11, s71: 11, s72: 12, s73: 15, s8: 16, s9: 17, s10: 19, ref: 20,
};
children.push(
  H1("목차"),
  GAP(160),
...
children.push(
  ...
  P("심정지 직전의 활력징후 궤적을 비지도 군집화하면 서로 다른 악화 아형이 자동으로 분리된다. 개발 데이터에서는 호흡성 악화형, 순환성 악화형, 혼합형의 세 아형이 확인되었다. 표현형별로 지배적인 활력징후 변화가 다르므로, 경보 임계값과 대응 프로토콜을 아형별로 차별화하는 확장이 가능하다."),
  ...FIG("vitals_phenotypes.png", 0.442, "그림 5. 심정지 표현형 군집 — 기저선 대비 변화량 기준 아형 분리", 560),
