...
children.push(
  ...
  P("위 수치는 본 제안서의 그림 3~6을 생성한 실행 기준이다. 합성 코호트를 재생성하면 AUPRC는 XGBoost 0.77~0.97, NEWS 0.42~0.63 범위에서 변동한다.", { size: 18, color: MUTED, before: 100, after: 120 }),
  P([
  ...
  ...FIG("vitals_pr_curve.png", 0.855, "그림 3. PR 곡선 비교 — ROC로는 보이지 않는 격차가 정밀도–재현율 평면에서 드러난다", 360),
  ...FIG("vitals_alarm_burden.png", 0.712, "그림 4. 동일 검출률에서의 알람 부담 — 고민감도 구간에서 NEWS의 알람이 급증한다", 420),
  ...FIG("vitals_trajectory.png", 0.544, "그림 5. 환자 악화 궤적과 경보 시점 — 위험도 상승이 사건에 선행한다", 510),
  ...FIG("vitals_lead_time.png", 0.711, "그림 6. lead-time 분포 — 개입 가능 시간의 확보 정도", 415),
);
...
children.push(
  ...
  ...FIG("vitals_phenotypes.png", 0.442, "그림 7. 심정지 표현형 군집 — 기저선 대비 변화량 기준 아형 분리", 545),
...
children.push(
  ...
  ...FIG("dashboard_screenshot.png", 0.656, "그림 8. 구현·배포된 조기경보 화면 — 위험도 추이, 활력징후, 기여 요인, 알람 부담을 한 화면에 제시", 545),
...
children.push(
  ...
  P("“알람이 40% 줄었다”는 표현은 그 자체로는 체감되지 않는다. 병동 규모로 환산하면 다음과 같다. 아래는 제7-1장에서 실측한 검출률 50% 지점의 알람 부담(윈도우 100개당 본 모델 24.8건, NEWS 41.3건)을 시간당 1회 평가하는 병동에 적용한 것이다."),
  TABLE(
    [1700, 1900, 1850, 1850, 1726],
    ["병동 규모", "일일 평가 횟수", "NEWS 기준 알람", "본 모델 기준 알람", "감소"],
    [
      ["20병상", "480회", "198건 / 일", [{ t: "119건 / 일", b: true }], [{ t: "79건", b: true, c: ACCENT }]],
      ["30병상", "720회", "297건 / 일", [{ t: "179건 / 일", b: true }], [{ t: "118건", b: true, c: ACCENT }]],
      ["50병상", "1,200회", "496건 / 일", [{ t: "298건 / 일", b: true }], [{ t: "198건", b: true, c: ACCENT }]],
    ],
    { align: [AlignmentType.CENTER, AlignmentType.CENTER, AlignmentType.CENTER, AlignmentType.CENTER, AlignmentType.CENTER],
      headerAlign: [AlignmentType.CENTER, AlignmentType.CENTER, AlignmentType.CENTER, AlignmentType.CENTER, AlignmentType.CENTER] },
  ),
  P([
    { t: "30병상 병동 기준 하루 약 " },
    { t: "118건", b: true },
