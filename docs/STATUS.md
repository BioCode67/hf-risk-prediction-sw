...
3. **극단 상태에서 점수가 역전됩니다.** SpO2는 88~92%에서 위험 1.40배까지 오르다가
   80% 미만에서 0.93 → 0.00 → 0.54배로 꺾입니다(70~80% 구간 206건 중 발병 0건,
   test 분할 기준). 이미 붕괴한 환자는 발병 시점을 지났거나 다른 원인으로 악화 중이라
   패혈증 확률이 실제로 낮습니다. 대조군인 호흡수(2.13배)·체온(1.90배)은 단조 증가하므로
   SpO2 고유의 현상입니다. 모델은 맞지만 화면이 "안정"으로 읽히면 위험합니다.
   대시보드는 모델과 무관한 생리학적 채널로 이 경우 경고를 띄웁니다.
   수치 근거와 정정 이력은 [`sepsis-dashboard.md`](sepsis-dashboard.md) §4.
