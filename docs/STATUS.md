...
> | 연산 자원 | KOREN (HPC / AI Cloud / VM) 사용 가능 |
...
- [ ] **MIMIC-IV 심정지 실학습** — CITI 승인이 났으므로 이제 가능. KOREN에서
      `python src/mimic_explore.py <경로> --model --gpu`. 성공하면 제안서 §7-2의
      합성 수치를 **실제 심정지 데이터 실측치로 교체** — 실현성 항목 최대 강화
- [ ] **§3 상용 제품 실명 확인** — 활력징후 기반 심정지 예측 국내 상용 솔루션의
      정식 명칭·인허가 현황·공개 성능 지표를 조사해 표에 명기하고 출처 표기.
      선행 사례 누락은 조사 부족으로 읽힘
- [ ] **데이콘 문의** — 데이터셋 2개 이상 결합 허용 여부 (게시판 권장: 답변이 문서로 남음)
- [ ] 제안서 최종화: 30장 분량 확장, PDF 변환, 데이터셋명 명기, 참고문헌 출처 확정
      — §7-3의 조건 3가지를 반드시 유지할 것
