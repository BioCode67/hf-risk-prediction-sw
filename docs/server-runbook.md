...
> 이 문서는 **실제로 겪은 함정과 해결책**을 그대로 담았습니다.
>
> ⚠️ 이 서버는 **개발·검증용**입니다. 대회 **본선 안심존은 완전히 별개**(오프라인 폐쇄망·CPU·
...
### ⚠️ 먼저 알아둘 것 — 이 컨테이너의 제약

이 서버는 **CUDA만 설치된 맨 컨테이너**입니다. 처음 접속하면 이렇습니다:
...
→ 그래서 **`/workspace`에 Miniconda를 직접 설치**해서 씁니다. root 권한이 필요 없고,
...
conda install -y -c conda-forge git    # ⚠️ hf 환경 안에서 설치해야 합니다
git --version
```
...
2. 플랫폼을 물으면 **Linux** (로컬 PC가 아니라 **서버**의 OS입니다)
...
> ⚠️ **이 서버는 SSH 포트 포워딩이 막혀 있어 터널 방식이 작동하지 않습니다.**
...
> 근본 해결을 원하면 서버 관리자에게 **`AllowTcpForwarding yes` 허용** 또는 **8888 포트 매핑**을
...
> ⚠️ **`--horizon=6`을 반드시 붙이세요.** 기본값 1시간은 환자당 양성 윈도우가 1개뿐이라
