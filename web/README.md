...
부르는 모델은 `GROQ_MODEL`로 바꿉니다. **이 값은 언젠가 반드시 깨집니다** — 호스팅
모델 이름은 제공자 일정에 따라 사라지고, 실제로 `llama-3.3-70b-versatile`은
2026-06-17부로 무료·개발자 티어에서 폐기됐습니다. 그래서 화면의 "자연어 설명"에
지금 부를 모델 이름을 적어 두고, 실패 메시지에도 그 이름을 넣습니다. 400/404로
실패하기 시작하면 코드가 아니라 이 환경변수를 바꾸세요. 설정 예시는
`.env.local.example`, 쓸 수 있는 이름은 <https://console.groq.com/docs/models>.

바꿀 때 `src/vitals_narrate.py`의 `DEFAULT_MODEL`도 같이 맞추세요 — 같은 역할을
하는 상수가 두 곳에 있습니다.
...
src/
  ...
  components/
    patient-list.tsx            # 병동 목록 + 검색 + 필터(발병/불일치) + 정렬
  ...
  lib/
    ...
    url.ts                      # 보고 있는 화면을 쿼리스트링에 싣고 되읽기
```

## 화면을 그대로 공유하기

지금 보고 있는 상태가 주소창에 실립니다. 링크를 그대로 보내면 상대도 같은 화면을
봅니다. 새로고침해도 유지됩니다.

| 파라미터 | 뜻 | 예 |
|---|---|---|
| `p` | 환자 번호 | `?p=184` |
| `h` | 고정한 시점(기록 시작 후 경과 시간) | `&h=28` |
| `t` | 경보 임계값(확률) | `&t=0.004` |

없는 값은 기본값으로 채워집니다 — 환자는 위험도 1위, 임계값은 스냅샷에 구워진
운영점입니다. 히스토리는 쌓지 않으므로(`replaceState`) 슬라이더를 움직여도 뒤로가기가
망가지지 않습니다.

## 키보드

병동 목록은 탭 정지점 하나만 차지합니다. 400행을 탭으로 통과해야 다음 패널에
닿는다면 없느니만 못하기 때문입니다.

- `Tab` — 목록으로 진입 (현재 선택된 행)
- `↑` / `↓` — 행 이동
- `Enter` / `Space` — 그 환자 열기
- 열 제목(`환자` / `위험도` / `NEWS`)은 버튼이며, 누르면 정렬이 바뀝니다
...
- 상태 색은 두 벌입니다. **칠**(`--success`/`--warning`/`--serious`/`--critical`)은
  면적·음영·아이콘용이고, **글자**(`--status-*`)는 양쪽 테마에서 4.5:1을 넘도록 따로
  잡은 판입니다. 글자에 칠 색을 쓰면 라이트 모드에서 `--warning`이 1.8:1까지 떨어집니다.
